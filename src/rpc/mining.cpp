...
UniValue generate(const UniValue& params, bool fHelp)
{
    ...
    {
        std::unique_ptr<CBlockTemplate> pblocktemplate(BlockAssembler(Params()).CreateNewBlock(minerAddress, nullopt, nZsfDepositAmount));
...
//            "  \"coinbasevalue\" : n,               (numeric) maximum allowable input to coinbase transaction, including the generation award and transaction fees (in " + MINOR_CURRENCY_UNIT + ")\n"
    ...
    const CAmount nIssuanceReserve{chainActive.Tip()->GetIssuanceReserve()};
    ...
    for (const CTransaction& tx : pblock->vtx) {
        ...
        if (tx.IsCoinBase()) {
            ...
            if (!canopyActive && nextHeight > 0 && nextHeight <= consensus.GetLastFoundersRewardBlockHeight(nextHeight)) {
                CAmount nBlockSubsidy = consensus.GetBlockSubsidy(nextHeight, nIssuanceReserve);
...
UniValue getblocksubsidy(const UniValue& params, bool fHelp)
{
    ...
    const CAmount nIssuanceReserve{(blockIndex == nullptr) ? MAX_MONEY : blockIndex->GetIssuanceReserve()};
    CAmount nBlockSubsidy = consensus.GetBlockSubsidy(nHeight, nIssuanceReserve);
